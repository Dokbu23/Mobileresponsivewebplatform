...
export function VirtualTourModal({
  ...
  const scenes: Tour360Scene[] = useMemo(() => {
    if (customScenes && customScenes.length > 0) {
      return customScenes.map((slot: any, index: number) => {
        let pano = slot.panoramaUrl || slot.imageUrl || '';
        if (pano && !pano.startsWith('http') && !pano.startsWith('blob:') && !pano.startsWith('data:')) {
          pano = `${API_BASE}${pano}`;
        }

        // If hotspots already built, preserve them; otherwise auto-build walking hotspots
        let hotSpots = slot.hotSpots;
        if (!hotSpots || hotSpots.length === 0) {
          hotSpots = [];
          if (index < customScenes.length - 1) {
            const next = customScenes[index + 1];
            hotSpots.push({
              pitch: -4,
              yaw: 20,
              text: `Walk to ${next.title || 'Next Spot'} ➡️`,
              type: 'scene',
          ...
          if (index > 0) {
            const prev = customScenes[index - 1];
            hotSpots.push({
              pitch: -5,
              yaw: -160,
              text: `⬅️ Return to ${prev.title || 'Previous Spot'}`,
              type: 'scene',
