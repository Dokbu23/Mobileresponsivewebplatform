      ...
      <div className="flex items-end flex-col-reverse">
        {open && (
          <div className="mb-2 sm:mb-3 w-[calc(100vw-2rem)] sm:w-[420px] max-w-[calc(100vw-2rem)] sm:max-w-[calc(100vw-2.5rem)] h-[540px] max-h-[82vh] bg-white border border-gray-200 rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slideUp">
            ...
            {/* ── Chat Header ── */}
            <div className="px-5 py-4 text-white flex items-center justify-between shadow-md relative bg-gradient-to-r from-pink-600 via-rose-600 to-indigo-700">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="flex items-center justify-center h-10 w-10 rounded-2xl shadow-inner text-white bg-white/20 backdrop-blur-md">
            ...
            <div ref={listRef} className="p-4 flex-1 overflow-y-auto bg-gradient-to-b from-gray-50/50 to-white space-y-3.5 chat-scroll">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex items-end gap-2.5 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fadeIn`}>
                  {msg.sender === 'bot' && (
                    <div className="w-8 h-8 rounded-2xl flex items-center justify-center text-white flex-shrink-0 shadow-xs bg-gradient-to-br from-pink-500 to-rose-600">
              ...
              {sending && (
                <div className="flex items-end gap-2.5 justify-start animate-fadeIn">
                  <div className="w-8 h-8 rounded-2xl flex items-center justify-center text-white flex-shrink-0 bg-gradient-to-br from-pink-500 to-rose-600">
                    <Bot className="h-4 w-4" />
                  </div>
        ...
        <button 
          ...
          className="p-3 sm:p-4 rounded-2xl sm:rounded-3xl shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 relative group flex items-center justify-center chat-launcher-btn bg-gradient-to-r from-pink-600 via-rose-600 to-indigo-600 text-white shadow-pink-500/20"
        >
          {!open && (
            <div className="absolute -top-1 -right-1 h-4 w-4 sm:h-5 sm:w-5 rounded-full flex items-center justify-center text-[9px] sm:text-[10px] font-black border-2 border-white shadow-xs animate-bounce bg-emerald-500 text-white">
              💬
            </div>
          )}
