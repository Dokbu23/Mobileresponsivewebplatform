...
export function BusinessProfile() {
  ...
  const { userType, currentUser, setCurrentUser, isInWishlist, removeFromWishlist, addToWishlist, getWishlistCount } = useApp();
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-8">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
              ...
              <AutoSwipeCarousel
                images={
                  Array.isArray(viewingProduct.images) && viewingProduct.images.length > 0
                    ? viewingProduct.images.map((img: string) => formatImageUrl(img) || img).filter(Boolean)
                    : [formatImageUrl(viewingProduct.image) || viewingProduct.image].filter(Boolean)
                }
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <span className="px-3 py-1 bg-pink-100 text-pink-700 text-xs font-bold rounded-full">
                    {viewingProduct.category || 'Local Product'}
                  </span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {/* Share Button */}
                  <button
                    onClick={() => setProductShareData({
                      title: viewingProduct.name,
                      description: viewingProduct.description,
                      image: viewingProduct.image,
                      category: viewingProduct.category || 'Product',
                    })}
                    className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-pink-50 transition-colors"
                    title="Share product"
                  >
              ...
              {/* Sub-info bar: saves · store · owner · directions */}
              <div className="flex flex-wrap items-center justify-between gap-2 bg-gray-50 p-3 rounded-2xl border border-gray-100 text-xs">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-gray-500 font-bold">({getWishlistCount(viewingProduct.id, 'product', viewingProduct.likes || 0)} saves)</span>
                  <span>•</span>
                  <div className="font-bold text-pink-600 flex items-center gap-1">
                    <Store className="h-3.5 w-3.5 text-pink-500" />
                    <span>{shopName}</span>
                  </div>
                  {(viewingProduct.owner?.name || viewingProduct.productOwner) && (
                    <>
                      <span>•</span>
                      <span className="text-gray-600 font-medium">
                        Owner: <strong className="text-gray-900">{viewingProduct.owner?.name || viewingProduct.productOwner}</strong>
                      </span>
                    </>
                  )}
                </div>
                <button
                  onClick={() => {
                    const query = encodeURIComponent(`${shopName} Mansalay Oriental Mindoro`);
              ...
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-2xl font-black text-pink-600 ml-2">₱{Number(viewingProduct.price || 0).toLocaleString()}</span>
              </div>

              {/* Description */}
              <div className="text-xs text-gray-600 leading-relaxed space-y-2" style={{ textAlign: 'justify' }}>
                {viewingProduct.description ? (
                  viewingProduct.description
                    ...
                    .map((para: string, idx: number) => (
                      <p key={idx} className="text-justify leading-relaxed" style={{ textAlign: 'justify' }}>{para}</p>
                    ))
                ) : (
              ...
              <div className="bg-pink-50/50 border border-pink-100 p-4 rounded-2xl text-center space-y-3">
                ...
                <div className="flex items-center justify-center gap-2 flex-wrap">
                  {owner?.phone && (
                    <a
                      href={`tel:${owner.phone}`}
                      className="px-4 py-2 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 rounded-full text-xs font-bold transition-colors flex items-center gap-1.5"
                    >
                  ...
                  {owner?.facebook_link && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="px-4 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded-full text-xs font-bold transition-colors flex items-center gap-1.5"
                    >
      ...
      {/* ── SHARE MODAL for Product ── */}
      {productShareData && (
        <ShareModal
          isOpen={!!productShareData}
          onClose={() => setProductShareData(null)}
          title={productShareData.title}
          description={productShareData.description}
          image={productShareData.image}
          category={productShareData.category}
        />
      )}
